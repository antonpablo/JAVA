...
import es.ucm.fdi.tp.practica6.cliente.GameClient;
...
public class Main {
	...
	private enum ModoJuego{
		NORMAL("normal"), CLIENT("client"), SERVER("server");
	 ...
	 * Juego por defecto.
	 */
	final private static ModoJuego DEFAULT_MODE = ModoJuego.NORMAL;
	...
	private static Integer NumObstaculos;
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructObstaculosOption());//-o or --obstacles
		cmdLineOptions.addOption(constructMode());
		cmdLineOptions.addOption(constructPort());
		cmdLineOptions.addOption(constructHost());
		//contruir el puerto el loscalhost y el modo
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case TicTacToe:
			...
			break;
		case ATAXX:
			if (dimRows != null && dimCols != null && dimRows == dimCols && NumObstaculos!=null) {
				gameFactory = new AtaxxFactoryExt(dimRows,NumObstaculos);
			} else {
	...
	/**
	 * Constructor  de la opci�n comando para introducir por argumentos el numero de obstaculos
	 * @return option de obstaculos
	 */
	private static Option constructObstaculosOption() {
		return new Option("o", "obstacles", true,
				"Contruir obstaculos con un numero no mayor al 10% del tablero.");
	}
	
	/**
	 * realiza el parse del comando abstaculo el cual parsea el numero de obstaculos que
	 * hay en el tablero
	 * @param line
	 * @throws ParseException
	 */
	private static void parseObsOption(CommandLine line) throws ParseException {
		String numObs = line.getOptionValue("o");
		if (numObs != null) {
			try {
				NumObstaculos= Integer.parseInt(numObs);
				
			} catch (NumberFormatException e) {
				throw new ParseException("numero de obstaculos invalidos " + numObs);
			}
		}

	}
	private static Option constructMode() {
		// TODO Auto-generated method stub
		return new Option("am", "app-mode", true,
				"modo de juego: normal, servidor o cliente");
	}

	private static void parseModeOptions(CommandLine line) throws ParseException {
		// TODO Auto-generated method stub
		String modeVal = line.getOptionValue("am", DEFAULT_MODE.getId());
		// view type
		for (ModoJuego g : ModoJuego.values()) {
			if (modeVal.equals(g.getId())) {
				mode = g;
			}
		}
		if (mode == null) {
			throw new ParseException("Uknown modelo '" + modeVal + "'");
		}
		
	}

	private static Option constructHost() {
		// TODO Auto-generated method stub
		return new Option("sh", "server-host", true,
				"hosting para la opcion de red");
			
	}

	private static void parseHostOptions(CommandLine line) {
		// TODO Auto-generated method stub
	...
	private static Option constructPort() {
		// TODO Auto-generated method stub
		return new Option("sp", "server-port", true,
				"puerto para la opcion de red");
	}
	private static void parsePortOptions(CommandLine line) {
		// TODO Auto-generated method stub
		...
		if (port != null) {
			...
			} catch (NumberFormatException e) { System.err.println("error numero de puerto");
			}
		}
	}
	...
	private static void startClient() {
		try {
			...
			}catch(Exception e){System.err.println("error vista!!!");}
